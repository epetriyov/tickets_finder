...
дороже заданной цены; каждая строка уведомления — прямая ссылка, кладущая
эти места в корзину виджета.

Настройка на любое событие — две строки в `.env`:

```
TARGET_URL=https://afisha.yandex.ru/moscow/concert/basta-2026-08-29
SESSION_DATE=2026-08-29   # обязательна, если у события несколько дат
```

Ключ сеанса и название события бот определяет сам со страницы события
(ключи сеансов вшиты в её HTML). Если автоопределение сломается — можно
задать `SESSION_KEY` вручную (README ниже, «Если API изменится»).

Критерии поиска — тоже в `.env`: `SECTORS` (какие сектора), `SEATS_NEEDED`
(сколько мест рядом), `MAX_PRICE_PER_TICKET` (максимум за билет,
включительно), `IGNORE_LIMITED_VIEW`.

⚠️ После правки `.env` на сервере делай `docker compose up -d
--force-recreate watcher`, а не `restart`: env-переменные вшиваются в
контейнер при создании, restart их не перечитывает.
