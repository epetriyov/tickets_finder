...
| `HEARTBEAT_HOURS` | `0` (выключен) | период «я жив»-сообщений, можно дробное; `0`/пусто = выключить |
...
- **heartbeat** (опционально, выключен по умолчанию) — раз в `HEARTBEAT_HOURS`:
  «💓 работает: проверок N, ошибок M, вариантов сейчас K»;
...
| Автоопределение перестало находить УЖЕ известный сеанс (наблюдалось 20.07.2026) | ближе к распродаже Яндекс перестаёт вшивать ключ сеанса в SSR-HTML страницы события (сама вкладка даты в UI остаётся, а ключ пропадает) — при этом ключ сеанса продолжает работать в API виджета. Если сеанс уже был известен — пропиши его явно в `SESSION_KEY`, авторезолвинг больше не нужен |
